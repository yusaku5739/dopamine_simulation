...
#define _ion_caski	*_ppvar[1]._pval
...
static void nrn_alloc(Prop* _prop) {
 ...
 prop_ion = need_memb(_cask_sym);
 nrn_promote(prop_ion, 1, 0);
 	_ppvar[1]._pval = &prop_ion->param[1]; /* caski */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 1, "cask_ion");
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 kca /home/workspace/266814/newkca.mod\n");
...
/*CVODE*/
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
   double _lminf ;
 _lminf = 1.0 / ( 1.0 + pow ( km / cai , n ) ) ;
    m = m + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tausk)))*(- ( ( ( _lminf ) ) / tausk ) / ( ( ( ( - 1.0 ) ) ) / tausk ) - m) ;
   }
  return 0;
}
...
static void _ode_matsol(NrnThread* _nt, _Memb_list* _ml, int _type) {
 ...
 static void _update_ion_pointer(Datum* _ppvar) {
   ...
   nrn_update_ion_pointer(_cask_sym, _ppvar, 1, 1);
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/workspace/266814/newkca.mod";
static const char* nmodl_file_text = 
